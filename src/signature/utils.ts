...
import { ECSignature, EIP712Object, EIP712ObjectValue, EIP712TypedData, EIP712Types } from './types';
 ...
 */
export const send = (provider, data) => new Promise((resolve, reject) => provider.sendAsync(data, (err, result) => {
    if (result.error) {
        err = result.error;
    }
    if (err) {
